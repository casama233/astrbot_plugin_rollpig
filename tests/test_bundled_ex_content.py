...
def test_bundled_release_keeps_quarantined_authored_packs_absent():
    ...
    assert set(explicit) == HANDWRITTEN_IDS
    assert HANDWRITTEN_IDS == set(pig_by_id)
    assert effective
...
def test_handwritten_pigs_use_explicit_copy_instead_of_generic_phase_suffixes():
...
def test_every_bundled_pig_now_uses_explicit_handwritten_copy():
    _, pig_by_id, explicit, effective = _catalog_and_effective_variants()

    assert set(explicit) == set(pig_by_id) == HANDWRITTEN_IDS
    for pig_id in HANDWRITTEN_IDS:
        assert effective[pig_id] == explicit[pig_id]
