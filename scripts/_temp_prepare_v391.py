...
def tag_date(version: str) -> str:
    try:
        value = subprocess.check_output(
            ["git", "show", "-s", "--format=%cs", f"v{version}"],
            cwd=ROOT,
            text=True,
        ...
        return RELEASE_DATE
    return value if re.fullmatch(r"\d{4}-\d{2}-\d{2}", value) else RELEASE_DATE


def release_body(version: str) -> str:
    path = ROOT / ".github" / f"release-v{version}.md"
    if not path.is_file():
        raise SystemExit(f"missing historical release notes: {path}")
...
def update_metadata() -> None:
    ...
    if count != 1:
        raise SystemExit("expected exactly one current v3.9.0 metadata version")
    path.write_text(text, encoding="utf-8")


def update_release_files() -> None:
    (ROOT / ".github" / "release-v3.9.1.md").write_text(RELEASE_NOTES, encoding="utf-8")

    (ROOT / "tests" / "test_v390_release_metadata.py").write_text(
    ...
    (ROOT / "tests" / "test_v391_release_metadata.py").write_text(
...
def rebuild_changelog() -> None:
    ...
    if pos < 0:
        raise SystemExit("CHANGELOG.md missing v3.6.5 history boundary")
    legacy_tail = text[pos:].lstrip()

    history_versions = ["3.9.0", "3.8.1", "3.8.0", "3.7.3", "3.7.2", "3.7.1", "3.7.0"]
    history = [
        f"## v{version} ({tag_date(version)})\n\n{release_body(version)}"
        for version in history_versions
    ]
    current_body = RELEASE_NOTES.split("\n", 1)[1].strip()
    rebuilt = (
        "# 更新\n\n"
        "## 未發佈\n\n"
        "- 暫無。\n\n"
