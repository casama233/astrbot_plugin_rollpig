...
def load_bundled_ex_copy(
    ...
    *,
    ...
    path = Path(resource_dir) / BUNDLED_EX_COPY_FILENAME
    if not path.is_file():
        return {}

    payload = json.loads(path.read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict):
        raise ValueError("Bundled EX 手写文案规格必须是 JSON 对象")

    provenance = payload.get("provenance")
    if not isinstance(provenance, dict):
        raise ValueError("Bundled EX 手写文案规格缺少 provenance")
    if str(provenance.get("scope") or "") != BUNDLED_EX_COPY_SCOPE:
        raise ValueError("Bundled EX 手写文案 provenance scope 不匹配")
    if provenance.get("quarantined_ex_used") is not False:
        raise ValueError("Bundled EX 手写文案必须声明 quarantined_ex_used=false")

    allowlist = {str(item) for item in bundled_ids if str(item)}
    specs = _normalize_specs(payload.get("pigs"))
