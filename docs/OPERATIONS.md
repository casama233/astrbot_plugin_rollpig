...
v3.12.3 起，公共 Vercel／GitHub 鏡像只接受獨立批准清單中的完整快照。來源為已完成審核及正式發布的 2026.09.05.1（164 隻／820 級 EX）；主源優先、私人源隔離、較舊快照拒絕降級。批准表不可讀時保留本地已驗證快取，不啟用新鏡像；讀到撤回後移出對應 mirror active 快取。詳見 [公共災備邊界](PUBLIC-MIRROR-FAIL-CLOSED.md)。

管理頁分開顯示「當前資源」的快取來源與「可用遠端」來源鏈。快取來自哪裡不代表目前仍允許下載；舊快取缺少來源記錄時明確標示未知，不從當前設定推測來源。公共鏡像封鎖亦單獨顯示。

手動同步在背景執行，頁面不自動輪詢。點右上角「刷新」會重新讀取狀態和圖鑑；「任務已啟動」只表示已受理，成功與失敗以讀取到的最新狀態為準。同步失敗保留現有資源，關閉自動同步的提示不會遮蓋錯誤。
...
工作台以「已安裝」標示磁碟版本，並區分尚未檢查、更新操作失敗、已成功檢查及等待重載。只有成功檢查取得完整 Release 結果後才顯示最新版本結論與 SHA-256 狀態；失敗保留上次結果、發行說明和成功檢查時間，但先停用安裝按鈕，重新檢查成功後才可繼續。已安裝但待重載時不重複提供同一更新。
