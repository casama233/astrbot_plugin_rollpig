...
本文面向插件管理員與維護者，說明 v3.3.0 的身份遷移、資料存儲、資源同步、安全更新、備份與常見故障處理。
...
v3.3.0 預設關閉自動同步，且不預填 manifest。啟用前請在插件配置填寫你有權使用的 HTTPS 私人源；同步間隔預設為 24 小時。
...
本地資源頁還可把本地小豬提交到 PigHub 人工審核。每次操作都要求管理員明確確認，只發送名稱與圖片，不發送描述、完整文案、群友資料或聊天內容。插件不會自動輪詢審核結果。
