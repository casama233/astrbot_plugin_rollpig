...
這三個權重就是 **60 / 30 / 10**。Roast Charge、預約、添柴都不會偷偷把它改成 80 / 15 / 5。

!!! info "添柴不是成功率 Buff"
    預約裡多人添柴目前代表群聊參與與 Gameplay Event，**不會把成功率堆高**。到目標真正觸發預約時，仍然是 60/30/10。
...
這是前端展示，用 60/30/10 權重讓你直觀看結果分布；**它不調用插件、不讀 Charge，也不是 AstrBot 真實 RNG**。
...
**60/30/10 本身目前不是配置項。** 它由核心 Roast policy 統一提供，避免不同入口各玩各的。
