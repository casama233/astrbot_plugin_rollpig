...
本文對應目前主線的 `_conf_schema.json`。推薦透過 AstrBot 插件配置介面修改；除非你清楚 AstrBot 的配置載入方式，否則不要直接改寫運行時配置檔。
...
| `roast_reservation_max_participants` | int | `12` | `2-20` | 每張預約包含固定主廚在內的最大參與人數；後續玩家再次 `/烤群友 @同一目標` 即可免費加入蹲守，不消耗自己的普通冷卻。 |
...
| `group_roast_cooldown_hours` | float | `8` | `1-72` | 普通烤群友按「發起者 + 群組」計算冷卻；第一位主廚建立預約時消耗一次，後續加入蹲守與預約觸發不重複扣除。後門可繞過。 |
...
| `oven_refill_support_ratio_percent` | int | `30` | `1-100` | 發起 `/烤箱補貨` 時按當刻今日活躍人數計算本輪支持門檻；本輪進行中新增活躍玩家不會重算門檻。 |
| `oven_refill_min_supporters` | int | `3` | `2-20` | 補貨最低支持人數；2 人小群固定需要 2 人。 |
| `oven_refill_max_base_supporters` | int | `8` | `3-50` | 基礎支持人數上限；後續成功輪次仍可按設定增加難度。 |
| `oven_refill_extra_supporters_per_success` | int | `2` | `0-10` | 同群同日每成功一輪後，下一輪額外需要的支持人數。 |
| `oven_refill_round_timeout_minutes` | int | `120` | `5-720` | 單輪補貨有效時間；超時仍未達標會自動關閉，需重新 `/烤箱補貨`。 |

### 預約烤豬建議

預約只在明確指定未抽豬目標時建立。主廚先支付普通冷卻，目標在同群顯示自己的今日小豬時才結算；**不存在 `/添柴` 指令**。後續玩家若要加入預約，請再次使用 `/烤群友 @同一目標`。加入人數目前不增加成功率。詳細行為見 [`ROAST-RESERVATIONS.md`](ROAST-RESERVATIONS.md)。

### 群體烤箱補貨

`/烤箱補貨` 負責建立本群當輪補貨，發起者自動算第一份支持；其餘今日活躍玩家使用 `/添煤` 支持。同一玩家每輪只計一次。門檻在**發起當刻**固定，因此後來新增的活躍玩家不會改變本輪已公示的所需人數；達標結算時則仍會按當刻活躍玩家恢復能量。
