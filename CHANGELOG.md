...
- 大幅翻修管理面板資訊層級與視覺密度：累計抽取不再重複繪製與每日活躍近乎等價的 mini 趨勢，今日活躍明確標示近 14 日範圍，熱門小豬改為緊湊排行榜卡片並消除大片無效留白。
- AI 烤豬文案運行健康改為狀態驅動呈現：後端向聚合 Analytics 暴露安全的啟用狀態，前端區分未啟用、已啟用無樣本、生成中與已有完成樣本；未啟用時不再顯示 0 成功／0 失敗／0% 成功率造成誤解。
- 新增管理面板大翻修回歸契約，鎖定資源版本相容、AI 狀態語義、累計抽取呈現與熱門榜版式。
