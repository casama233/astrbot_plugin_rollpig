...
- EX 成長管理的「實際生效預覽」現在同步顯示最終生效圖片與來源；選擇本地圖片可在儲存前即時預覽，勾選移除圖片會即時模擬稀疏繼承、公共／內建 EX 或基礎圖的最終回退結果。
