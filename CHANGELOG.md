...
- 所有管理页资源缓存键同步提升至 v3.0.2，不修改 SQLite 单一权威、API 契約或業務流程。
...
- 兼容 JSON 仍在同一事务内同步，用于导出、旧版回滚和災難恢復；上述熱路徑不再觸發對應投影全表刪除重建。
...
- 新增醒目的“等待重启”横幅；待重启期间禁用迁移、验证、重建、导出、回滚 JSON、同步与更新按钮。
